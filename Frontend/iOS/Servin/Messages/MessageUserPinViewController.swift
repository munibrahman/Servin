...
class MessageUserPinViewController: UIViewController {
    ...
    @objc func userDidTapSave() {
        ...
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate, let appSyncClient = appDelegate.appSyncClient else {
            return
        }
        
        guard let pin = discovery, let discoveryId = pin.discoveryId else {
            print("Cant unwrap pin")
            return
        }
        
        if pin.isSaved ?? false {
            saveButton.image = #imageLiteral(resourceName: "save_icon_empty")
            appSyncClient.perform(mutation: RemoveSavedDiscoveryMutation.init(discoveryId: discoveryId))
        } else {
            saveButton.image = #imageLiteral(resourceName: "save_icon_fill")
            
            appSyncClient.perform(mutation: SaveDiscoveryMutation.init(discoveryId: discoveryId))
        }
...
extension MessageUserPinViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if self.discovery?.author?.username == AWSMobileClient.sharedInstance().username {
