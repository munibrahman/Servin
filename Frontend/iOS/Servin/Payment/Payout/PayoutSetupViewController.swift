...
// Description: This view controller guides a user through setting up their bank account (External account).
 ...
 class PayoutAddressViewController: UIViewController, UITextFieldDelegate, ScrollViewControllerDelegate {
    ...
    @objc func userDidPressNext() {
        ...
        guard let address = addressField.text, !address.isEmpty else {
            self.showWarningNotification(title: "Warning", subtitle: "Address field can't be empty!")
            return
        }
        
        guard let city = cityField.text, !city.isEmpty else {
            self.showWarningNotification(title: "Warning", subtitle: "City field can't be empty!")
            return
        }
        
        guard let postalCode = postalcodeField.text, !postalCode.isEmpty else {
            self.showWarningNotification(title: "Warning", subtitle: "Postal code can't be empty!")
            return
        }
        
        guard let province = provinceField.text, !province.isEmpty else {
            self.showWarningNotification(title: "Warning", subtitle: "Province can't be empty!")
            return
        }
...
private class PayoutFullNameViewController: UIViewController, UITextFieldDelegate {
    ...
    @objc func userDidPressNext() {
        ...
        guard let fname = firstNameField.text, !fname.isEmpty else {
            self.showWarningNotification(title: "Warning", subtitle: "First name can't be empty!")
            return
        }
        
        guard let lname = lastNameField.text, !lname.isEmpty else {
            self.showWarningNotification(title: "Warning", subtitle: "Last name can't be empty!")
            return
        }
        
        guard let bday = bdayField.text, !bday.isEmpty else {
            self.showWarningNotification(title: "Warning", subtitle: "Birthday can't be empty!")
            return
        }
        
        guard let sin = sinField.text, !sin.isEmpty else {
            self.showWarningNotification(title: "Warning", subtitle: "SIN number can't be empty!")
            return
        }
        
        print("go next")
        let viewController = PayoutBankAccountInfoViewController.PayoutBankAccountInfoViewControllerFor(fname: fname, lname: lname)
...
class PayoutBankAccountInfoViewController: UIViewController, UITextFieldDelegate {
    ...
    @objc func userDidPressNext() {
        ...
        guard let fname = fname else {
            print("First name is incorrenct")
            self.showWarningNotification(title: "Warning", subtitle: "First name is incorrect")
            return
        }
        
        guard let lname = lname else {
            print("Last name is incorrenct")
            self.showWarningNotification(title: "Warning", subtitle: "Last name is incorrect")
            return
        }
        
        guard let transitNumber = transitField.text, !transitNumber.isEmpty else {
            print("Show error that transit number is empty")
            self.showWarningNotification(title: "Warning", subtitle: "Transit field is empty")
            return
        }
        
        guard let instNumber = instututionField.text, !instNumber.isEmpty else {
            print("Show error that institution number is empty")
            self.showWarningNotification(title: "Warning", subtitle: "Institution field is empty")
            return
        }
        
        guard let accountNumber = accountField.text, !accountNumber.isEmpty else {
            print("Show error that account number is empty")
            self.showWarningNotification(title: "Warning", subtitle: "Account field is empty")
            return
        }
        
        
        
        VolticStripeHelper.shared.createExternalAccount(name: fname + " " + lname,
                                                        transit: transitNumber,
                                                        institution: instNumber,
                                                        account: accountNumber,
                                                        onSuccess: {
            self.showSuccessNotification(title: "Success!", subtitle: "Bank account added successfully!")
            ...
            print(error)
            self.showErrorNotification(title: "Error", subtitle: "Unable to add bank acocunt, please try again")
        }
    ...
    static func PayoutBankAccountInfoViewControllerFor(fname: String, lname: String) -> PayoutBankAccountInfoViewController {
        let viewController = PayoutBankAccountInfoViewController()
        viewController.fname = fname
        viewController.lname = lname
