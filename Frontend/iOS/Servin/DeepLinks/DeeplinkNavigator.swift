    ...
    private func confirmSignUp(email: String, code: String) {
        ...
        AWSMobileClient.sharedInstance().confirmSignUp(username: email, confirmationCode: code) { (signUpResult, error) in
            if let signUpResult = signUpResult {
                ...
                UIApplication.topViewController()?.showErrorNotification(title: "Error", subtitle: MobileClientHelper.decipher(error: error))
                ...
                print(error)
                print("\(error.localizedDescription)")
                UIApplication.topViewController()?.showErrorNotification(title: "Error", subtitle: "Unable to verify your email, please try again.")
                UIApplication.topViewController()?.hideServinLogoProgressView()
            }
        }
    }
